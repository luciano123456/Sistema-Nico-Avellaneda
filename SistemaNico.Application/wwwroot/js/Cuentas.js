...
async function toggleEstado(icon) {
    ...
    const monedaSelect = card.querySelector('.pdv-moneda-select');
    const idMoneda = parseInt(monedaSelect.value);

    const nuevoEstado = !activo;

    const res = await fetch("/Cuentas/Actualizar", {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ id, nombre, activo: nuevoEstado ? 1 : 0, idMoneda })
    });
...
async function cargarCuentas() {
    const contenedor = document.querySelector("#contenedorCuentas");
    if (!contenedor) return;

    contenedor.innerHTML = "";

    const [puntosResponse, monedasResponse] = await Promise.all([
        fetch("/Cuentas/Lista"),
        fetch("/Monedas/Lista")
    ]);
    ...
    puntos.forEach(p => {
        ...
        const opcionesMonedas = monedas.map(m =>
            `<option value="${m.Id}" ${m.Id === p.IdMoneda ? 'selected' : ''}>${m.Nombre}</option>`
        ).join("");
        ...
        col.innerHTML = `
            <div class="card card-pdv text-center animate__animated animate__fadeIn ${claseEstado}" data-id="${p.Id}">
                ...
                <div class="card-body d-flex flex-column align-items-center justify-content-center px-3 pb-4 pt-2">
                    <i class="fa fa-university fa-4x mb-3 text-white"></i>
                    <input type="text" class="pdv-nombre-input mb-2 text-center" value="${p.Nombre}" disabled />
                    <select class="form-select mb-3 pdv-moneda-select" disabled>
                        ${opcionesMonedas}
                    </select>
    ...
    agregarCardNuevo(contenedor, monedas);
}

function agregarCardNuevo(contenedor, monedas) {
    const opcionesMonedas = monedas.map(m =>
    ...
    col.innerHTML = `
        <div class="card card-pdv tarjeta-agregar text-center animate__animated animate__fadeIn">
            <div class="card-front d-flex flex-column align-items-center justify-content-center h-100" onclick="mostrarFormularioAgregar()">
                ...
                <p class="text-white mb-0">Agregar Cuenta</p>
            </div>
            <div class="card-back d-none text-center h-100 d-flex flex-column justify-content-between">
                ...
                <div class="card-body d-flex flex-column align-items-center justify-content-center px-3 pb-4 pt-2">
                    <i class="fa fa-university fa-4x mb-3 text-white"></i>
                    <input type="text" class="pdv-nombre-input pdv-nombre-input-editable mb-2 text-center" placeholder="Nueva Cuenta" />
                    <select class="form-select mb-3 pdv-moneda-select">
                        ${opcionesMonedas}
                    </select>
                </div>
            </div>
...
function editarNombre(icon) {
    const card = icon.closest(".card-pdv");
    const input = card.querySelector(".pdv-nombre-input");
    const select = card.querySelector(".pdv-moneda-select");
    ...
    btnGuardar.classList.add("text-success");
...
async function guardarNombre(icon) {
    ...
    const nombre = card.querySelector(".pdv-nombre-input").value.trim();
    const idMoneda = parseInt(card.querySelector(".pdv-moneda-select").value);
    const activo = card.classList.contains("pdv-activo") ? 1 : 0;

    const res = await fetch("/Cuentas/Actualizar", {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ id, nombre, activo, idMoneda })
    });
...
async function eliminarPDV(btn) {
    ...
    const res = await fetch(`/Cuentas/Eliminar?id=${id}`, { method: "DELETE" });
...
async function insertarNuevoPDV(btn) {
    ...
    if (nombre === "") {
        errorModal("Debe ingresar un nombre.");
        return;
    }

    const res = await fetch("/Cuentas/Insertar", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ id: 0, nombre, activo: 1, idMoneda })
    });
